...
namespace CDStoreManagement.Utils
{
    ...
    class ExportToExcelM
    {
        private static ExportToExcelM instance;
        public static ExportToExcelM Instance
        {
            get
            {
                if (instance == null)
                    instance = new ExportToExcelM();
                return instance;
            }
        }
        private ExportToExcelM() { }
        Action<Excel.Workbook, string, Excel.XlFileFormat> saveFile;
        public bool Export(DataGridView dataGridView, string path, ModeExcelORPDF mode = ModeExcelORPDF.XLS)
        {
            saveFile = ToExcel;
            ...
            switch (mode)
            {
                case ModeExcelORPDF.XLSX:
                    fileFormat = Excel.XlFileFormat.xlOpenXMLWorkbook;
                    break;
                case ModeExcelORPDF.PDF:
                    saveFile = ToPDF;
                    break;
                default:
                    break;
